<template>
    ...
    >
      ...
      <el-table-column prop="menuType" label="菜单类型" width="120"
        ...
        <template v-slot="scope">
          ...
          <el-tag size="small" v-else-if="scope.row.menuType === 'C'"
                  type="success" effect="dark">菜单
          </el-tag>
          <el-tag size="small" v-else-if="scope.row.menuType === 'F'"
                  type="warning" effect="dark">按钮
          </el-tag>
        </template>
      </el-table-column>
  ...
  <Dialog v-model="dialogVisible" :tableData="tableData" :id="id" :dialogTitle="dialogTitle" @initMenuList="initMenuList"/>
</template>
